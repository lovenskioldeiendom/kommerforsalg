...
def _ensure_tables(conn):
    conn.executescript("""
        ...
        CREATE INDEX IF NOT EXISTS idx_snap_fylke ON snapshots(fylke);
...
def save_snapshot(municipality: str | None, fylke: str | None,
                  project: dict, project_url: str) -> None:
    ...
    with get_conn() as conn:
        conn.execute("""
            INSERT INTO snapshots
              (date, municipality, fylke, finn_code, title, subtitle, address,
        ...
        """, (
            today, municipality, fylke, project.get("finn_code"),
...
def get_latest_snapshots() -> list[dict]:
    ...
    with get_conn() as conn:
        rows = conn.execute("""
            ...
            ORDER BY s.fylke, s.municipality, s.title
        """).fetchall()
